...
async function loadProfileSettingsAsync() {
    ...
    try {
        ...
        _profileCache = {
            username: data.displayName || '@usuario',
...
async function saveProfileSettings(settings) {
    ...
    try {
        await QDApi.updateProfile({
            displayName: settings.username || settings.displayName || '@usuario',
...
function applyProfileDataToView(settings) {
    if (profileUsername) {
        profileUsername.textContent = settings.username || '@usuario';
    }

    if (profileUsernameInput) {
        profileUsernameInput.value = settings.username || '@usuario';
    }
...
if (profilePhotoInput && profilePhotoPreview && photoPlaceholder) {
    ...
    if (saveIdentityButton) {
        saveIdentityButton.addEventListener('click', async () => {
            ...
            settings.username = usernameValue || '@usuario';
            settings.displayName = usernameValue || '@usuario';
