...
import { BoundedCache, ErrorHandler } from './utils.js';
...
const _notesCache = new BoundedCache(200); // Max 200 notes cached
let _cachedNotes = [];
...
async function loadProfileSettings() {
    ...
    if (profileStore?.loadProfile) {
        _cachedProfile = await profileStore.loadProfile();
        return _cachedProfile;
    }

    const defaults = { username: 'Seu nome', realName: '', identityNotes: '', responsePrompt: '', synthesisPrompt: '' };
    try {
        const data = await QDApi.getProfile();
        const resolvedUsername = profileStore?.resolveDisplayName
            ? profileStore.resolveDisplayName(data)
            : ((typeof data.displayName === 'string' && data.displayName.trim()) || (typeof data.username === 'string' && data.username.trim()) || '')
            || defaults.username;
        _cachedProfile = {
