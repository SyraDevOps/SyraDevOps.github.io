...
// ========== TESTES DE PERFIL/AUTH ==========
runner.describe('Profile Auth', () => {
  runner.it('Store de perfil deve estar disponivel', () => {
    assertExists(window.QDProfileStore, 'QDProfileStore deve existir em window');
    assert(typeof window.QDProfileStore.loadProfile === 'function', 'Store deve ter loadProfile()');
  });

  runner.it('Deve normalizar username com @', () => {
    const cleaned = window.QDProfileStore.normalizeUsername('@usuario_teste');
    assertEquals(cleaned, 'usuario_teste', 'Deve remover @ do inicio');
  });

  runner.it('Deve resolver nome por prioridade', () => {
    const resolved = window.QDProfileStore.resolveDisplayName(
      { username: '' },
      { displayName: '  maria  ' },
      { username: 'nao_deve_usar' }
    );
    assertEquals(resolved, 'maria', 'Deve usar o primeiro nome valido da lista');
  });

  runner.it('Deve usar fallback amigavel no perfil padrao', () => {
    const defaults = window.QDProfileStore.getDefaultProfile();
    assertExists(defaults.displayName, 'Perfil padrao deve ter displayName');
    assert(defaults.displayName.length > 0, 'DisplayName padrao nao pode ser vazio');
  });
});
...
// ========== TESTES DE UTILS.JS (NOVOS) ==========
runner.describe('🛠️ Utils & Utilities', () => {
  runner.it('Deve normalizar username corretamente', async () => {
    const { normalizeUsername } = await import('./utils.js');
    
    assertEquals(normalizeUsername('@usuario'), 'usuario', 'Deve remover @');
    assertEquals(normalizeUsername('  spaces  '), 'spaces', 'Deve fazer trim');
    assertEquals(normalizeUsername(''), '', 'String vazia deve retornar vazia');
    assertEquals(normalizeUsername(null), '', 'Null deve retornar string vazia');
  });

  runner.it('StorageAdapter deve salvar e recuperar dados', async () => {
    const { StorageAdapter } = await import('./utils.js');
    
    const testKey = 'test_storage_key';
    const testValue = { name: 'Test', value: 123 };
    
    const saved = StorageAdapter.set(testKey, testValue);
    assert(saved === true, 'Deve salvar com sucesso');
    
    const retrieved = StorageAdapter.get(testKey);
    assertEquals(retrieved.name, testValue.name, 'Deve recuperar nome');
    assertEquals(retrieved.value, testValue.value, 'Deve recuperar valor');
    
    StorageAdapter.remove(testKey);
    const afterRemove = StorageAdapter.get(testKey);
    assert(afterRemove === null, 'Deve retornar null após remoção');
  });

  runner.it('BoundedCache deve limitar tamanho', async () => {
    const { BoundedCache } = await import('./utils.js');
    ...
    assertEquals(cache.size, 3, 'Deve ter 3 items');
    
    cache.set('key4', 'value4');
    assertEquals(cache.size, 3, 'Deve manter máximo de 3 items');
    assert(!cache.has('key1'), 'Deve remover primeiro item (FIFO)');
    assert(cache.has('key4'), 'Deve ter novo item');
  });

  runner.it('Retry logic deve tentar múltiplas vezes', async () => {
    const { retryAsync } = await import('./utils.js');
    ...
    const result = await retryAsync(failTwiceThenSucceed, 3, 10);
    assertEquals(result, 'Sucesso', 'Deve retornar sucesso após retries');
    assertEquals(attempts, 3, 'Deve ter tentado 3 vezes');
  });

  runner.it('Debounce deve atrasar execução', async () => {
    ...
    assertEquals(callCount, 0, 'Não deve executar imediatamente');
    
    await new Promise(resolve => setTimeout(resolve, 100));
    assertEquals(callCount, 1, 'Deve executar apenas uma vez após delay');
  });

  runner.it('ErrorHandler deve inicializar container', async () => {
    const { ErrorHandler } = await import('./utils.js');
    
    ErrorHandler.init();
    const container = document.getElementById('toast-container');
    assert(container !== null, 'Deve criar container de toasts');
  });
});

// ========== TESTES DE INTEGRAÇÃO API ==========
runner.describe('🔌 API Integration', () => {
  runner.it('QDApi deve estar disponível globalmente', () => {
    assertExists(window.QDApi || QDApi, 'QDApi deve existir');
  });

  runner.it('Deve ter métodos HTTP básicos', () => {
    const api = window.QDApi || QDApi;
    assert(typeof api._fetch === 'function', 'Deve ter _fetch');
    assert(typeof api._get === 'function', 'Deve ter _get');
    assert(typeof api._post === 'function', 'Deve ter _post');
  });

  runner.it('Deve gerenciar estado de autenticação', () => {
    const api = window.QDApi || QDApi;
    assert(typeof api.isAuthenticated === 'boolean', 'isAuthenticated deve ser boolean');
    assert(typeof api.setAuth === 'function', 'Deve ter setAuth()');
    assert(typeof api.clearAuth === 'function', 'Deve ter clearAuth()');
  });

  runner.it('Deve ter offline queue', () => {
    const api = window.QDApi || QDApi;
    assert(typeof api.processOfflineQueue === 'function', 'Deve ter processOfflineQueue()');
  });
});

// ========== TESTES DE PERFORMANCE ==========
runner.describe('⚡ Performance', () => {
  runner.it('Service Worker deve estar registrado', async () => {
    if ('serviceWorker' in navigator) {
      const registration = await navigator.serviceWorker.getRegistration();
      assert(registration !== undefined, 'Service Worker deve estar registrado');
      console.log('  ℹ️ SW Scope:', registration?.scope);
    } else {
  ...
  runner.it('LocalStorage deve estar disponível', () => {
    assert(typeof localStorage !== 'undefined', 'localStorage deve existir');
    
    const testKey = 'perf_test';
    localStorage.setItem(testKey, 'test');
    const value = localStorage.getItem(testKey);
    assertEquals(value, 'test', 'localStorage deve funcionar');
    localStorage.removeItem(testKey);
  });

  runner.it('Fetch API deve estar disponível', () => {
    assert(typeof fetch === 'function', 'fetch deve estar disponível');
  });

  runner.it('Web Crypto API deve estar disponível', () => {
    assert(typeof crypto !== 'undefined', 'crypto deve existir');
    assert(typeof crypto.subtle !== 'undefined', 'crypto.subtle deve existir');
  });
});
