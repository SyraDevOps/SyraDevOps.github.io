...
let user, profile = {}, marks = [], selected, editing, draftPin, pendingDeepLink;
...
map.on('load', () => {
  ...
  map.addLayer({ id: 'cluster-halo', type: 'circle', source: 'community-marks', filter: ['has', 'point_count'], paint: { 'circle-radius': ['step', ['get', 'point_count'], 26, 20, 32, 100, 39], 'circle-color': '#43c7ff', 'circle-opacity': 0.16, 'circle-blur': 0.55 } });
  map.addLayer({ id: 'clusters', type: 'circle', source: 'community-marks', filter: ['has', 'point_count'], paint: { 'circle-color': ['step', ['get', 'point_count'], '#198ff2', 20, '#6578fa', 100, '#b779ff'], 'circle-radius': ['step', ['get', 'point_count'], 18, 20, 23, 100, 29], 'circle-stroke-color': '#dff8ff', 'circle-stroke-width': 1.5 } });
  ...
  map.addLayer({ id: 'mark-halo', type: 'circle', source: 'community-marks', filter: ['!', ['has', 'point_count']], paint: { 'circle-radius': ['interpolate', ['linear'], ['get', 'likes'], 0, 10, 5, 15, 15, 20, 22, 40, 29], 'circle-color': ['case', ['>=', ['get', 'likes'], 40], '#ffd166', ['>=', ['get', 'likes'], 15], '#ff8c42', ['>=', ['get', 'likes'], 5], '#c77dff', '#28bdfb'], 'circle-opacity': 0.22, 'circle-blur': 0.35 } });
  map.addLayer({ id: 'unclustered-mark', type: 'circle', source: 'community-marks', filter: ['!', ['has', 'point_count']], paint: { 'circle-radius': ['interpolate', ['linear'], ['get', 'likes'], 0, 6, 5, 9, 15, 12, 40, 16], 'circle-color': ['case', ['>=', ['get', 'likes'], 40], '#ffd166', ['>=', ['get', 'likes'], 15], '#ff8c42', ['>=', ['get', 'likes'], 5], '#c77dff', ['>=', ['get', 'likes'], 1], '#278bff', '#21bfff'], 'circle-stroke-color': '#ffffff', 'circle-stroke-width': 1.5, 'circle-opacity': ['case', ['==', ['get', 'visibility'], 'reduced'], 0.42, 1] } });
  sourceReady = true; redraw();
  map.on('click', 'clusters', event => { const feature = event.features[0], source = map.getSource('community-marks'); source.getClusterExpansionZoom(feature.properties.cluster_id, (error, zoom) => { if (!error) map.easeTo({ center: feature.geometry.coordinates, zoom }); }); });
  map.on('click', 'unclustered-mark', event => { const mark = marks.find(item => item.id === event.features[0].properties.id); if (mark) openMark(mark); });
...
function addDraft(coordinates) {
  ...
  draftPin.on('dragend', () => { const point = draftPin.getLngLat(); selected = { lat: point.lat, lng: point.lng }; updateCoords(); });
}
...
function dialog(coordinates, mark) {
  ...
  $('#markTitle').value = mark?.title || ''; $('#markDescription').value = mark?.description || ''; $('#category').value = mark?.category || 'place'; $('#life').value = mark?.life || 'permanent'; $('#markPhoto').value = mark?.imageURL || '';
...
$('#newMark').onclick = () => dialog(map.getCenter()); $('#life').onchange = setPulseVisibility;
document.querySelectorAll('[data-close]').forEach(button => button.onclick = closeDialog);
map.on('click', event => { if ($('#markDialog').classList.contains('hidden')) dialog(event.lngLat); else { selected = { lat: event.lngLat.lat, lng: event.lngLat.lng }; draftPin?.setLngLat(event.lngLat); updateCoords(); } });
['search', 'filterCategory', 'filterLife'].forEach(id => $(`#${id}`).addEventListener('input', redraw));

function actionButton(label, handler, className = '') { const button = document.createElement('button'); button.textContent = label; button.className = className; button.onclick = handler; return button; }
...
async function openMark(mark) {
  ...
  const meta = document.createElement('p'); meta.className = 'mark-meta'; meta.textContent = `${categoryNames[mark.category] || 'Informação'} · ${mark.life === 'pulse' ? `Pulso até ${new Date(ts(mark.expiresAt)).toLocaleString()}` : 'Marco permanente'} · ${mark.author?.displayName || 'Explorador'}`; root.append(meta); appendInfo(root, mark);
  ...
  actions.append(actionButton('Curtir', () => toggleInteraction('likes', mark)));
  if (mark.authorId !== user.uid) actions.append(actionButton('Confirmar', () => toggleInteraction('confirmations', mark)));
  actions.append(actionButton('Compartilhar', () => shareMark(mark)));
  ...
  if (mark.authorId === user.uid) { actions.append(actionButton('Editar', () => { popup.remove(); dialog({ lat: mark.location.lat, lng: mark.location.lng }, mark); })); actions.append(actionButton('Apagar', async () => { if (confirm('Apagar esta marcação?')) await deleteDoc(doc(db, 'markers', mark.id)); }, 'danger')); }
  root.append(actions);
...
async function shareMark(mark) {
  const url = `${location.origin}${location.pathname}?mark=${encodeURIComponent(mark.id)}`; const share = { title: mark.title, text: `${categoryNames[mark.category] || 'Marcação'} no GeoMark: ${mark.title}`, url };
...
function renderFeed() {
  const host = $('#feedItems'); const activity = [...filteredMarks()].sort((a, b) => Math.max(ts(b.lastConfirmedAt), ts(b.createdAt)) - Math.max(ts(a.lastConfirmedAt), ts(a.createdAt))).slice(0, 7);
...
$('#markForm').onsubmit = async event => {
  ...
  try {
    ...
    const data = { title, description, category: $('#category').value, life, durationHours: life === 'pulse' ? hours : null, expiresAt: life === 'pulse' ? Timestamp.fromMillis(Date.now() + pulseMs[hours]) : null, location: { lat: selected.lat, lng: selected.lng }, imageURL: safeUrl($('#markPhoto').value.trim()), updatedAt: serverTimestamp() };
